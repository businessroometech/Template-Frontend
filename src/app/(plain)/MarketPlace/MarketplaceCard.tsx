...
const [WishlistDeleted , setWishlistDeleted] = useState(false)
    ...
    const handleWishlistDelete = async () => {
        const response = await fetch(`http://100.29.20.215:5000/wishlists/delete/${Wishlists.data[0].id}`, {
            method: "DELETE",
        });
    ...
    const handleWishlist = async () => {
        try {
            ...
            const response = await fetch("http://100.29.20.215:5000/wishlists/create" , {
                method : "POST",
                headers : {
    ...
    const handledelete = async() => {
        try {
            if (user?.id == business.UserId) {
                const response = await fetch(`http://100.29.20.215:5000/businessseller/delete/${business.id}`, {
                    method: "DELETE",
                });
                console.log(await response.json())
                setIsDeleted(true);
